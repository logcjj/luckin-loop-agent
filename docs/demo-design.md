...
1. 桌面端左侧是任务、最近对话，以及生活、记忆·进化、商家、接入·设置入口；中间是大的聊天/引导主窗口；右侧是方案、画像、执行、数据画布。
...
- 顶部：产品名、模拟数据、取餐 ETA、距离，以及右上角网页/手机切换。
- 左侧桌面栏：新的咖啡任务、最近对话，以及生活、记忆·进化、商家、接入·设置入口和支付前停止边界。
- 中间工作区：聊天起手、需求引导、商品方案卡、门店/券/兜底说明、底部输入；提交后不回显用户原文。
- 右侧画布：方案、画像、执行、数据四个 tab，放置候选召回、会员画像、GUI-Agent cockpit 和真实数据接入蓝图。
