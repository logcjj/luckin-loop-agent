...
默认打开是用户视图网页：中间是一块大的聊天窗口，不让用户先选场景，也不展示固定推荐。小鹿先追问口味、状态、甜度、取餐时间，再给商品方案卡、真实咖啡图、到手价、取餐 ETA 和定位授权/拒绝定位 fallback。切到手机后才显示一台手机里的聊天与点单模拟器。

### 第二幕 · 给稳方案

右上角可切到商家视角。商家视角按小满结构组织：左侧是新的咖啡任务、最近对话、生活、记忆·进化、商家、接入·设置；中间仍是大的聊天/引导主窗口；右侧是方案、门店、订单、记忆、执行、设置画布。推荐、门店、券包、执行链路、护栏和数据来源都放在右侧辅助区里展开。
