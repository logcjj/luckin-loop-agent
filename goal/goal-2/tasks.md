...
## Task 10: 下载小满原项目并在其基础上修改

- 状态：completed
- 可验证目标：
  - 下载 `bcefghj/xiaoman` 原仓库到本地临时/参考目录。
  - 检查 license、前端结构、核心文件，确认可作为底座迁入或改写。
  - 抛弃当前 CoffeePlan React 壳，改为基于小满前端结构的瑞幸版本。
  - 保留必要业务需求：瑞幸咖啡对话引导、候选方向、真实商品图、门店/订单/记忆/执行画布、Qwen 本地代理、支付前停止。
  - 不提交下载缓存、用户 key、node_modules、dist、Playwright 临时文件。
- 完成记录：
